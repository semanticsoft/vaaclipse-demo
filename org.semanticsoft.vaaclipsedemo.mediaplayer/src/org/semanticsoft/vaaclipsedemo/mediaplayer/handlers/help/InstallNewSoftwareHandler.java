...
public class InstallNewSoftwareHandler {
	...
	public void execute(UI ui, IContainerP2Views basicUI) {
		 ...
		 * window.setContent(mainLayout); window.setModal(true);
		 * window.setPositionX(500); window.setPositionY(100);
		 * mainLayout.setWidth("600px"); mainLayout.setHeight("450px");
		 * 
		 ...
		 * 
		 * CssLayout cssLayout = new CssLayout(); cssLayout.addComponent(label);
		 ...
		 * treeTable.setPageLength(5); treeTable.setWidth("40em");
		 ...
		 * 
		 * mainLayout.addComponent(textArea); textArea.setImmediate(true);
		 * 
		 * textArea.setEnabled(true);
		 * 
		 * textArea.setWidth("40em"); textArea.setHeight("7em"); Button button =
		 * new Button("Install"); button.setWidth("300px");
		 ...
		 * 
		 * final List<CheckBox> listChecks = new ArrayList<>(); final
		 * HashMap<String, List<CheckBox>> childsLinkedToRoot = new HashMap<>();
		 ...
		 * 
		 * selectedRepository.clear(); for (CheckBox checkBox2 : listChecks) {
		 * 
		 * Boolean value2 = checkBox2.getValue(); String caption2 =
		 * checkBox2.getCaption(); for (IInstallableUnit iInstallableUnit2 :
		 * loadRepository) {
		 * 
		 * if (caption2.equals(iInstallableUnit2 .getId())) {
		 * 
		 * if (value2) { selectedRepository .add(iInstallableUnit2); } boolean
		 * category = installService .isCategory(iInstallableUnit2); if
		 * (category) { // check all childs List<CheckBox> list =
		 * childsLinkedToRoot .get(caption2);
		 * 
		 ...
		 * 
		 * listChecks.add(checkBox); treeTable.addItem(new Object[] { checkBox,
		 ...
		 * 
		 * listChecks.add(checkBoxChild); treeTable.addItem(new Object[] {
		 * checkBoxChild, iInstallableUnitChild.getVersion() .toString() },
		 * iInstallableUnitChild.getId());
		 * 
		 ...
		 * childsLinkedToRoot.put(iInstallableUnit.getId(), listCheckBoxChilds);
		 * 
		 * } } } });
		 * 
		 ...
		 * textArea.setValue("");
		 * 
		 * String installNewSoftware = "OK"; try { installNewSoftware =
		 * installService .installNewSoftware(selectedRepository);
		 * 
		 * } catch (Exception exception) {
		 * 
		 * exception.printStackTrace(); if (exception.getMessage().contains(
		 * "Profile id _SELF_ is not registered"))
		 * 
		 * textArea.setValue("You must export via .product file first"); else
